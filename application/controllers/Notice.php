...
class Notice extends CI_Controller {
	...
	public function getDrillDownBenefit($Country) {
        $data['benefit'] = $this->Benefit_Model->getBenefit($Country);
		$benefit= $data['benefit'];
        if ( $data['benefit'] != null)
            $this->load->view('benefitDetails', $data);
        else {
			$data['msg'] = "The Benefit  You Searched For Could Not Be Found, Please Try Again";
...
/*
	...
	*/	
		...
		$notice = array(
			"Country"=> $_POST["shortDescription"],
			"Holidays"=>$_POST["longDescription"],
		
			"Leavetype"=>$_POST["area"],
			"Details"=>$_POST["dateExp"]
	...
	public function getDrillDownNotice($noticeId) {
		...
		if ( $data['notice'] != null)
			...
			$data['msg'] = "The Country You Searched For Could Not Be Found, Please Try Again";
		 	$this->load->view('msgpage', $data);
		 }
    }
